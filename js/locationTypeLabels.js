const LOC_TYPE_LABELS = {
  // Food & Drink
  restaurant:          'Restaurants',
  cafe:                'Cafés',
  fast_food:           'Fast Food Restaurants',
  bar:                 'Bars',
  pub:                 'Pubs',
  biergarten:          'Beer Gardens',
  food_court:          'Food Courts',
  ice_cream:           'Ice Cream Parlours',
  confectionery:       'Confectionery Shops',
  chocolate:           'Chocolate Shops',
  coffee:              'Coffee Shops',
  tea:                 'Tea Shops',
  beverages:           'Beverage Stores',
  alcohol:             'Alcohol Stores',
  wine:                'Wine Shops',
  deli:                'Delis',
  bakery:              'Bakeries',
  butcher:             'Butchers',
  greengrocer:         'Greengrocers',
  fish:                'Fish Shops',
  health_food:         'Health Food Stores',

  // Groceries & Daily Needs
  supermarket:         'Supermarkets',
  convenience:         'Convenience Stores',
  kiosk:               'Kiosks',
  newsagent:           'Newsagents',
  tobacco:             'Tobacco Shops',
  variety_store:       'Variety Stores',
  discount:            'Discount Stores',

  // Health & Beauty
  pharmacy:            'Pharmacies',
  chemist:             'Drugstores',
  optician:            'Opticians',
  hairdresser:         'Hairdressers',
  beauty:              'Beauty Salons',
  massage:             'Massage Studios',
  tattoo:              'Tattoo Studios',
  laundry:             'Laundromats',
  dry_cleaning:        'Dry Cleaners',

  // Clothing & Accessories
  clothes:             'Clothing Stores',
  shoes:               'Shoe Stores',
  jewelry:             'Jewellery Stores',
  watches:             'Watch Stores',
  bag:                 'Bag Shops',

  // Electronics & Tech
  electronics:         'Electronics Stores',
  computer:            'Computer Stores',
  mobile_phone:        'Mobile Phone Stores',
  hifi:                'Hi-Fi Stores',
  video_games:         'Video Game Stores',

  // Home & Garden
  furniture:           'Furniture Stores',
  hardware:            'Hardware Stores',
  doityourself:        'DIY Stores',
  diy:                 'DIY Stores',
  garden_centre:       'Garden Centres',
  florist:             'Florists',
  interior_decoration: 'Interior Design Stores',
  bed:                 'Bed Shops',
  kitchen:             'Kitchen Shops',
  bathroom:            'Bathroom Shops',

  // Sports & Outdoor
  sports:              'Sports Stores',
  outdoor:             'Outdoor Stores',
  bicycle:             'Bicycle Shops',
  fishing:             'Fishing Shops',
  golf:                'Golf Stores',
  ski:                 'Ski Stores',
  diving:              'Diving Shops',

  // Books, Art & Hobbies
  books:               'Bookstores',
  stationery:          'Stationery Stores',
  art:                 'Art Shops',
  craft:               'Craft Shops',
  fabric:              'Fabric Shops',
  sewing:              'Sewing Shops',
  antiques:            'Antique Shops',
  second_hand:         'Second-Hand Shops',
  gift:                'Gift Shops',
  toys:                'Toy Stores',
  games:               'Game Shops',
  model:               'Model Shops',
  music:               'Music Stores',
  musical_instrument:  'Music Instrument Stores',
  photo:               'Photo Shops',
  video:               'Video Stores',

  // Pets
  pet:                 'Pet Shops',
  veterinary:          'Veterinary Clinics',

  // Services
  bank:                'Banks',
  bureau_de_change:    'Currency Exchange',
  post_office:         'Post Offices',
  travel_agency:       'Travel Agencies',
  car:                 'Car Dealerships',
  car_repair:          'Car Repair Shops',
  car_wash:            'Car Washes',
  fuel:                'Petrol Stations',

  // Department & Mall
  department_store:    'Department Stores',
  mall:                'Shopping Malls',
  marketplace:         'Marketplaces',
  market:              'Markets',
  indoor_market:       'Indoor Markets',

  // Education
  school:              'Schools',
  university:          'Universities',
  college:             'Colleges',
  kindergarten:        'Kindergartens',
  language_school:     'Language Schools',
  driving_school:      'Driving Schools',
  library:             'Libraries',

  // Healthcare
  hospital:            'Hospitals',
  clinic:              'Clinics',
  doctors:             'Doctor\'s Offices',
  dentist:             'Dental Clinics',
  physiotherapist:     'Physiotherapy Clinics',

  // Fitness & Leisure
  gym:                 'Gyms',
  fitness_centre:      'Fitness Centres',
  sports_centre:       'Sports Centres',
  sports_hall:         'Sports Halls',
  swimming_pool:       'Swimming Pools',
  ice_rink:            'Ice Rinks',
  bowling_alley:       'Bowling Alleys',
  sauna:               'Saunas',
  water_park:          'Water Parks',
  amusement_arcade:    'Amusement Arcades',
  escape_game:         'Escape Rooms',

  // Culture & Entertainment
  cinema:              'Cinemas',
  theatre:             'Theatres',
  museum:              'Museums',
  gallery:             'Art Galleries',
  arts_centre:         'Arts Centres',
  nightclub:           'Nightclubs',
  casino:              'Casinos',

  // Religion & Community
  place_of_worship:    'Places of Worship',
  church:              'Churches',
  mosque:              'Mosques',
  community_centre:    'Community Centres',
  social_centre:       'Social Centres',

  // Accommodation
  hotel:               'Hotels',
  hostel:              'Hostels',
  guest_house:         'Guest Houses',
  motel:               'Motels',
  dormitory:           'Dormitories',

  // Transport
  airport:             'Airports',
  train_station:       'Train Stations',
  bus_station:         'Bus Stations',
  subway_entrance:     'Subway Stations',
  ferry_terminal:      'Ferry Terminals',
  tram_stop:           'Tram Stops',

  // Government
  townhall:            'Town Halls',
  courthouse:          'Courthouses',
  police:              'Police Stations',
  fire_station:        'Fire Stations',

  // Work
  office:              'Offices',
  coworking_space:     'Coworking Spaces',

  // Social & Care
  nursing_home:        'Nursing Homes',
  childcare:           'Childcare Centres',
  social_facility:     'Social Facilities',
};
